...
$films_amount=array();
if(isset($_GET['year'])) {
	$sql="SELECT count(kinopoiskId) FROM films where year=".$_GET['year'];
	//order by kinopoiskId desc
	...
	$sql="SELECT * FROM films where year=".$_GET['year']." order by kinopoiskId desc";
	$result=mysqli_query($conn,$sql);
	if($result) {
	  while ($row = mysqli_fetch_row($result)) {
