...
if(isset($_GET['year'])) {
    ...
    $sql="SELECT * FROM films where nameRu is not null and genre like \"%".$_GET["genre"]."%\" and filmLength is not null and ratingAwait is null order by year desc";
    $result=mysqli_query($conn,$sql);
    if($result) {
        while ($row = mysqli_fetch_row($result)) {
            array_push($films_list,$row);
                  ...
                  <div class="row">
                     <div class="col-md-12">
                        <div class="main-title">
                           ...
                           <h6>Результаты поиска (найдено: <?php echo count($films_list);?>)</h6>
                        </div>
                     </div>
                      <?php
