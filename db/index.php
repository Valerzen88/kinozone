...
if(isset($_GET["addNewFilmsJob"])) {
    ...
    for($i=0;$i<$_GET["addNewFilmsJob"];$i++) {
        $sql = "SELECT MAX(kinopoiskId) FROM films where kinopoiskId<1100000";
        ...
        echo "<br>+++end job with " . $newlastId . " new lines in ".sprintf('%f', $exec_time)." +++<br>";
    }
}
...
function addStaff($startId,$amount_ids,$db,$conn){
    ...
    $personIds = array();
    $sql = "SELECT personId FROM staff where persondId between ".$startId." and ".$amount_ids;
    $result = mysqli_query($conn, $sql);
    if ($result) {
        while ($row = mysqli_fetch_row($result)) {
